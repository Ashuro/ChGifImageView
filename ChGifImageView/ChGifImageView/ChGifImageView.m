...
#ifdef  _CGLOG
#define CGLog(FORMAT, ...) printf("[%s]: %s\n", NSStringFromClass([self class]).UTF8String, [[NSString stringWithFormat:FORMAT, ##__VA_ARGS__] UTF8String]);
#define CGLogError(FORMAT,...) printf("[%s-error]: %s\n", NSStringFromClass([self class]).UTF8String, [[NSString stringWithFormat:FORMAT, ##__VA_ARGS__] UTF8String]);
#else
#define CGLog(FORMAT, ...) {}
#define CGLogError(FORMAT,...) {}
#endif
...
/// 承载图片的webView
@property (nonatomic,strong) UIWebView *playWebView;
/// 加载网络图片的风火轮
@property (nonatomic,strong) UIActivityIndicatorView *loadingIndicatorView;
/// 隐藏imageView
@property (nonatomic,assign) BOOL hiddenImageView;
/// 隐藏webView
@property (nonatomic,assign) BOOL hiddenWebView;
/// 图片总数
...
/// 切换图片的timer
@property (nonatomic,weak  ) NSTimer *chageImageTimer;
/// 切换图片的下标
@property (nonatomic,assign) NSInteger chageImageIndex;
/// 缓存策略
@property (nonatomic,assign) GifCacheType cacheType;

/// 快速循环的回调
@property (nonatomic,copy) void (^speedLoopCallBackBlock) (NSInteger index);
/// 快速循环完成回调
@property (nonatomic,copy) void (^speedLoopCompleteBlock) (id obj);
/// 点击视图回调
@property (nonatomic,copy) void (^tapCallBlock) (ChGifImageView * gifImageView);
...
//    CGLog(@"path:%@",DocumentPath);
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:ChGifCacheDirPath]) {
        ...
        if (!suc) {
            CGLogError(@"[ChGifImageView-ERROR]：缓存文件夹创建失败,请尝试重新创建");
        }
    }
...
#pragma unused(deleDir)
        ...
        if (!makeDir) {
            CGLogError(@"[ChGifImageView-ERROR]：图片缓存文件夹创建失败,请尝试重新调用【- (void) setImageName : (NSArray *) imageNameArray withRefreshType : (GifCacheType) refreshType】");
            return;
        }
    } else {
        // 检查缓存内是否存在图片缓存文件夹
...
- (void) chageImage {
    if ([self.imageType isEqualToString:@"gif"]) {
        ...
        if([[NSFileManager defaultManager] contentsOfDirectoryAtPath:ImageCacheDirPath(self.imageName) error:nil].count>0) {
            ...
            if (!img) {
                CGLogError(@"[ChGifImageView-ERROR]：从缓存内读取图片失败");
...
+ (void)refCache {
    ...
    if (deleDir) {
        CGLog(@"[ChGifImageView]：缓存清理成功");
    } else {
        CGLogError(@"[ChGifImageView-ERROR]：缓存清理失败");
    }
}
...
+ (void)refCacheWithImageName:(NSString *)gifName {
    ...
    if (deleDir) {
        CGLog(@"[ChGifImageView]：【%@】缓存清理成功",gifName);
    } else {
        CGLogError(@"[ChGifImageView-ERROR]：【%@】缓存清理失败",gifName);
    }
}
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    CGLogError(@"[ChGifImageView-ERROR]：网络图片加载失败-%@",error.localizedDescription);
}
